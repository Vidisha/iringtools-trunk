...
public class DirectoryProvider
{
  ...
  public void postDirectory(Directory directory, String user, String baseDir) throws ServiceProviderException
  {
    ...
    postDirectoryToLdap(directory, ldapProvider, user);
  }

  public void postDirectoryToLdap(Directory directory, LdapProvider ldapProvider, String user) throws ServiceProviderException
  {
    String base_dir1;
    ...
    try
    {
      ...
      {
        LdapFolder dirFolder = new LdapFolder(user, folder.getName(), folder.getDescription(), folder.getType(),
            folder.getContext(), getMember(folder.getGroup(), folder));
        base_dir1 = ldapProvider.storeDirectoryFolder(dirFolder, base_dir);
        traverseDirectory(folder, base_dir1, ldapProvider, user);
      }
    }
    catch (Exception e)
    {
  ...
  private String traverseDirectory(Folder folder, String base_dir, LdapProvider ldapProvider, String user) throws NamingException
  {
    ...
    if (endpoints != null)
    {
      ...
      {
        LdapEndpoint dirEndpoint = new LdapEndpoint(user, endpoint.getName(), endpoint.getDescription(), endpoint.getType(),
    ...
    else
    {
      ...
      {
        LdapFolder dirEntry = new LdapFolder(user, subFolder.getName(), subFolder.getDescription(), subFolder.getType(),
            subFolder.getContext(), getMember(subFolder.getGroup(), subFolder));
        baseDir1 = ldapProvider.storeDirectoryFolder(dirEntry, base_dir);
        traverseDirectory(subFolder, baseDir1, ldapProvider, user);
      }
    }
    return base_dir;
  }
