...
public class DirectoryXMLProvider extends ResourceProvider
{
  ...
  //private boolean findNodeOneLevel(Folder folder, String name, String type
  ...
  public Contexts getContextNames()
  {
  	...
  	if (directory != null)
			for (Folder folder : directory.getFolderList())
	  	{				
				addFolderContext(folder, contextList);
				collectContext(folder, contextList);					
	  	}	
  	
    return contexts;
  }
  
  private void addFolderContext(Folder folder, List<Context> contextList)
  {
  	Context context = null;
  	String contextStr = folder.getContext();
		if (!contextStr.isEmpty())
		{
			if (!containContext(contextList, contextStr))
			{
				context = new Context();
  ...
  private void collectContext(Folder folder, List<Context> contextList)
  { 
  	Folders folders = folder.getFolders();
  	if (folders != null)  	
  	{
  		for (Folder subFolder : folders.getItems())
  		{  
  			collectContext(subFolder, contextList);
  			addFolderContext(subFolder, contextList);   			
  		}
  	}  	  	
  }  
  ...
  public BaseUrls getBaseUrls()
  {
    ...
    if (directory != null)
			for (Folder folder : directory.getFolderList())
	  	{
				collectBaseUrl(folder, baseUrlList);					
	  	}	

    return baseUrls;
  }
  
  private void addEndpointUrl(Endpoint endpoint, List<BaseUrl> baseUrlList)
  {
  	BaseUrl baseUrl = null;
  	String baseUrlStr = endpoint.getBaseUrl();
		if (!baseUrlStr.isEmpty())
		{
			baseUrlStr = baseUrlStr + "/adapter";
			if (!containBaseUrl(baseUrlList, baseUrlStr))
			{
				baseUrl = new BaseUrl();				
				baseUrl.setUrl(baseUrlStr);
				baseUrlList.add(baseUrl);
			}		
		}
  }
  
  private void collectBaseUrl(Folder folder, List<BaseUrl> baseUrlList)
  {  	
  	Endpoints endpoints = folder.getEndpoints();
  	if (endpoints != null)
  	{
  		for (Endpoint endpoint : endpoints.getItems())
  		{  			
  			addEndpointUrl(endpoint, baseUrlList);
  		}
  	}
  	...
  	if (folders != null)  	
  	{
  		for (Folder subFolder : folders.getItems())
  		{  			
  			collectBaseUrl(subFolder, baseUrlList);
  		}
  	}  	
  }  
  ...
  {
  	if (folder.getFolders() == null || folder.getFolders().getItems().size() == 0)
  	{
