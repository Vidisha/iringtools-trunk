...
public class ResourceProvider
{
  ...
  protected boolean containBaseUrl (List<BaseUrl> baseUrlList, String baseUrlStr)
  {
  	for(BaseUrl baseUrl : baseUrlList)
  	{
  		if (baseUrl.getUrl().equalsIgnoreCase(baseUrlStr))
  			return true;
  	}
  	return false;
  }
  
  protected boolean containContext (List<Context> contextList, String contextStr)
  {
  	for(Context context : contextList)
  	{
  		if (context.getContext().equalsIgnoreCase(contextStr))
  			return true;
  	}
  	return false;
  }
