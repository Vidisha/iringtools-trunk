...
import org.iringtools.directory.BaseUrl;
import org.iringtools.directory.BaseUrls;
import org.iringtools.directory.Context;
import org.iringtools.directory.Contexts;
...
public class LdapProvider extends ResourceProvider
{
  ...
  {
    String user = "", type = "", name = "", description = "", context = "", baseUrl = "", assembly = "";
    ...
    try
    {
      ...
      {
      	user = removeHeader("user", attributes.get("user").toString());
      	treeNode.setUser(user);
      }
  ...
  {
    String user = "", type = "", description = "", name = ""; 
    String securityRole = "", id = "", context = "", baseUrl = "", assembly = "";
    ...
    for (Node child : node.getChildren())
    {
      ...
      if (type != null)
      {
        ...
        else if (type.compareTo("exchange") == 0)
        {
          ...
          addFolderToDirectory(user, type, description, securityRole, name, folder, directory, child, context);
      }
      else
      {
        addFolderToDirectory(user, type, description, securityRole, name, folder, directory, child, context);
  ...
  private void addFolderToDirectory(String user, String type, String description, String securityRole, String name, Folder folder,
  ...
  public void updateDirectoryNode(String user, String path, String name, String type, String description,
      String context, String baseUrl, String assembly) throws NamingException
  {    
    ...
    if (hasLdapNode(oldLdapFullName))
    {
      try
      {
        ...
        if (type.compareTo("folder") == 0)
        {
          ...
          mods[3] = new ModificationItem(DirContext.REPLACE_ATTRIBUTE, new BasicAttribute("user", user));
        ...
        else if (type.compareTo("endpoint") == 0)
        {
          ...
          mods[4] = new ModificationItem(DirContext.REPLACE_ATTRIBUTE, new BasicAttribute("user", user));
          dctx.modifyAttributes(oldLdapFullName, mods);          
        }
      }
      catch (Exception ex)
    ...
    else
    {
      if (type.compareTo("folder") == 0)
      {
        ...
        LdapFolder directoryEntry = new LdapFolder(user, name, description, type, context, memeber);
        bindLdapFolder(directoryEntry, newLdapFullName);        
      }
      else if (type.compareTo("endpoint") == 0)
      {
        String endpointContext = getParentContext(newLdapFullName);
        LdapEndpoint directoryEndpoint = new LdapEndpoint(user, name, description, type, endpointContext, baseUrl, assembly, "");
  ...
  public Contexts getContextNames()
  {
  	Contexts contexts = new Contexts();
  	String[] attrIDs = { "type", "context" };
    SearchControls ctls = new SearchControls();
    ctls.setReturningAttributes(attrIDs);
    ctls.setSearchScope(SearchControls.SUBTREE_SCOPE);
    ...
    contexts.setContextList(contextList);
    ...
    try
    {
      ...
      while (queryResourceResult.hasMore())
      {
        ...
        if (attrs.get("context") == null)
          continue;        
        
        contextName = getRealString(removeHeader("context", attrs.get("context").toString()));
        
        if (!contextName.isEmpty())
        {
        	if (!containContext(contextList, contextName))
        	{
  ...
  public BaseUrls getBaseUrls()
  {
    ...
    baseUrls.setBaseUrlList(baseUrlList);
    ...
    try
    {
      ...
      while (queryResourceResult.hasMore())
      {
        ...
        if (attrs.get("baseurl") == null)
          continue;        
        
        baseUrlValue = removeHeader("baseurl", attrs.get("baseurl").toString());
        
        if (!baseUrlValue.isEmpty())
        {
        	baseUrlValue = baseUrlValue + "/adapter";
        	if (!containBaseUrl(baseUrlList, baseUrlValue)) 
        	{
		        BaseUrl baseUrl = new BaseUrl();		        
		        baseUrl.setUrl(baseUrlValue);	        
