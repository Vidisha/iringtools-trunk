...
public class ExchangeProvider implements ServletContextListener
{
  private static final Logger logger = Logger.getLogger(ExchangeProvider.class);
  public static final String POOL_PREFIX = "_pool_";
  private static final ExecutorService execSvc = Executors.newFixedThreadPool(10);
  ...
  private Manifest createCrossedManifest() throws ServiceProviderException
  {
    ...
    Future<Manifest> sourceManifestTask = execSvc.submit(new ManifestTask(settings, sourceManifestUrl));    
    Future<Manifest> targetManifestTask = execSvc.submit(new ManifestTask(settings, targetManifestUrl));

    Manifest sourceManifest = null, targetManifest = null;
	try {
		sourceManifest = sourceManifestTask.get(Long.parseLong((String) settings.get("manifestTaskTimeout")), TimeUnit.SECONDS);
		targetManifest = targetManifestTask.get(Long.parseLong((String) settings.get("manifestTaskTimeout")), TimeUnit.SECONDS);
	} catch (InterruptedException e) {
  ...
  @Override
  // manual cleaning of threads goes here if necessary
