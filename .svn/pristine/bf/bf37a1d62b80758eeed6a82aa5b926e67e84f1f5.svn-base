    ...
    try
    {
      ...
      if (namespaceExist)
      {
        if (deleteFlag)
        {
          ...
          {
            if (repo.getNamespaceList() != null && repo.getNamespaceList().getItems().contains(nsID))
              repo.getNamespaceList().getItems().remove(nsID);
          }
        }
    ...
    try
    {
      ...
      for (Repository repository : _repositories)
      {
        ...
        for (Hashtable<String, String> result : results)
        {
          ...
          {
        	  resultEntity.setUri(result.get("rds"));
          }
