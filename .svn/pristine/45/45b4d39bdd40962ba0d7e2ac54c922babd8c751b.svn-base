...
public class OAuthFilter implements Filter 
{
  ...
  public static final String AUTHORIZATION = Constants.HTTP_HEADER_PREFIX + "Authorization";
  public static final String APP_KEY = Constants.HTTP_HEADER_PREFIX + "X-myPSN-AppKey";
  public static final String USER_ID = Constants.HTTP_HEADER_PREFIX + Constants.USER_ID_HEADER;
      ...
      ServletException
  {
    ...
    Cookie authCookie = HttpUtils.getCookie(request.getCookies(), AUTH_COOKIE);
      ...
      else  // case 2: the user has logged in but the application needs to process the SSO event
      {
        ...
        {  
          ...
          try
          {
            ...
            authCookie = new Cookie(AUTH_COOKIE, authCookieValue);
            authCookie.setMaxAge(AUTH_COOKIE_EXPIRY);
            response.addCookie(authCookie);
            
            session.setAttribute(AUTHORIZATION, userAttrs.get(OAUTH_TOKEN));
            
            String userId = getUserId(userAttrs);
            RequestWrapper reqWrapper = new RequestWrapper((HttpServletRequest)req, "", userId, ""); 
