...
public class DirectoryService extends AbstractService
{
  ...
  private String description = "", baseUrl = "", assembly = "", context = "", userId = "";
      ...
      @PathParam("type") String type, @PathParam("context") String context, String description)
  {
    ...
    {
      initService(SERVICE_NAME);
      prepareProvider(context, description, baseUrl, assembly);
      ...
      if (useLdap)
      {
        ...
        ldapProvider.updateDirectoryNode(userId, path, folderName, type, this.description, this.context, "", "");
      }
      else
      {
        DirectoryXMLProvider xmlProvider = new DirectoryXMLProvider(settings);
        ...
        xmlProvider.updateDirectoryNode(path, folderName, type, description, this.context, "", "");
      }
  ...
  {   
    try
    {
      ...
      if (useLdap)
      {
        ...
        directoryProvider.postDirectory(directory, userId, baseUrl);
      }
      else
      {
        DirectoryXMLProvider xmlProvider = new DirectoryXMLProvider(settings);
  ...
  @GET
  @Path("/directory/contextNames")
  @Produces(MediaType.APPLICATION_XML)
  public Response contextNames()
  {
  	try
    {
  		initService(SERVICE_NAME);      
      ...
      {
        LdapProvider ldapProvider = new LdapProvider(settings);
        contexts = ldapProvider.getContextNames();
      }
      else
      {
        DirectoryXMLProvider xmlProvider = new DirectoryXMLProvider(settings);
        contexts = xmlProvider.getContextNames();
      }      
  ...
  @GET
  @Path("/directory/baseUrls")
  @Produces(MediaType.APPLICATION_XML)
  public Response baseUrls()
  {
  	try
    {  	
      initService(SERVICE_NAME);
      ...
      {
        LdapProvider ldapProvider = new LdapProvider(settings);
        baseUrls = ldapProvider.getBaseUrls();
      }
      else
      {
        DirectoryXMLProvider xmlProvider = new DirectoryXMLProvider(settings);
        baseUrls = xmlProvider.getBaseUrls();
      }       
      
      return Response.ok().entity(baseUrls).build();
    }
    catch(Exception ex)
    {
      return prepareErrorResponse(HttpServletResponse.SC_INTERNAL_SERVER_ERROR, ex);
  ...
  private void prepareProvider(String context, String description, String baseUrl, String assembly)
  {
  	getUserLdap();
