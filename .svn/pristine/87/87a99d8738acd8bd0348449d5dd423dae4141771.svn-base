...
Ext.ux.plugin.PagingToolbarResizer = Ext.extend(Object, {
   ...
   * clicked. If you do not want the store to be automatically loaded the first
   * time the trigger is clicked, set to 'local' and manually load the store. To
   * force a requery of the store every time the trigger is clicked see
   * {@link #lastQuery}.
   * 
   * 'local' :
   * 
    ...
    var combo = new Ext.form.ComboBox({
      ...
      listeners : {
        ...
        keypress : function(textField, e) {
          if (e.getCharCode() == 13) {  // enter key
            pagingToolbar.pageSize = textField.value;
            pagingToolbar.doLoad(Math.floor(pagingToolbar.cursor / pagingToolbar.pageSize) * pagingToolbar.pageSize);
          }
        }*/
      }
